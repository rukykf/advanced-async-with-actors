...
#[message(result = "()")]
struct ProcessSymbolsMessage {
    symbol: String,
    from: DateTime<Utc>,
    to: DateTime<Utc>,
}

struct ProcessSymbolsActor;

impl Actor for ProcessSymbolsActor {}

#[async_trait::async_trait]
impl Handler<ProcessSymbolsMessage> for ProcessSymbolsActor {
    async fn handle(&mut self, _ctx: &mut Context<Self>, msg: ProcessSymbolsMessage) {
        handle_symbol_data(msg.symbol.as_str(), &msg.from, &msg.to).await;
    }
}
...
async fn main() -> std::io::Result<()> {
    ...
    let addr = ProcessSymbolsActor.start().await.unwrap();
